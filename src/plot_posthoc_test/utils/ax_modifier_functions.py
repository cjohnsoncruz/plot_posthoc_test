...
# Note: Some functions (set_stage_xticks_2_layer, get_move_legend_above_ax) 
# reference external helper_functions and sns_plotting_config modules.
# These are available when running notebooks but not part of the installed package. 
...
def get_move_legend_above_ax(plot_ax, bbox_to_anchor = None, **kwargs):
    ...
    #move to location
    # Note: sns_cfg.legend_defaults requires external sns_plotting_config module
    plot_ax.legend(hand, labs, bbox_to_anchor=bbox_to_anchor,loc = 'lower center', ncols = 4, frameon = False, **kwargs)
...
def ax_add_stage_names_and_outline_points(ax_obj, marker_edge_width=0.6, phase_list=None):
    """Note: Requires external sns_plotting_config for default parameters."""
    #tweak asthetics by 1) outlining pointplot objects and 2) adding the 2 layer scheme 
    set_pointplot_edgecolor(ax_obj, edge_color = 'black', linewidth = marker_edge_width)
    if phase_list is not None:
        sec, sec2, is_dict = set_stage_xticks_2_layer(ax_obj, phase_list)
...
def set_stage_xticks_2_layer(plot_ax, stage_order):
    """Note: Requires external helper_functions module for category processing.
    This function is only available when running with the full development environment."""
    raise NotImplementedError(
        "This function requires external helper_functions module. "
        "Use this in notebook environments with 'Function .py Storage' in sys.path."
    )
####### subfuncs
